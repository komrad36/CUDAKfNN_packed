...
int main() {
	...
	void* d_qvecs;
	cudaMalloc(&d_qvecs, 128 * size);
	cudaMemcpy(d_qvecs, uqvecs, 128 * size, cudaMemcpyHostToDevice);
	...
	resDesc.res.linear.sizeInBytes = 128 * size;
	...
	void* d_tvecs;
	cudaMalloc(&d_tvecs, 128 * (size + 8));
	cudaMemcpy(d_tvecs, utvecs, 128 * size, cudaMemcpyHostToDevice);
	...
	resDesc_train.res.linear.desc.f = cudaChannelFormatKindUnsigned;
	...
	resDesc_train.res.linear.sizeInBytes = 128 * size;
	...
	for (int i = 0; i < warmups; ++i) CUDAKfNN_packed(tex_t, size, tex_q, size, d_matches, threshold);
	std::cout << "Testing..." << std::endl;
	high_resolution_clock::time_point start = high_resolution_clock::now();
	for (int i = 0; i < runs; ++i) CUDAKfNN_packed(tex_t, size, tex_q, size, d_matches, threshold);
